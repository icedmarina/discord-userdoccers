...
^7^ Only included in [Gateway events](/gateway/gateway-events#channels) and when fetched from the [Get Private Channels](#get-private-channels) endpoint.
...
| type?                  | integer | The [type of thread](#channel-type) to create (default `PUBLIC_THRAD` and cannot be changed)                                                                                         |
| location?              | string  | The source of where the thread was created (e.g. message, plus button)                                                                                                               |
